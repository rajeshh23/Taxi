  ...
  setCol=true;
    constructor(props){
        ...
        if(sessionStorage.getItem("marketId")){
         ...
         this.state={
             ...
             tabIndex:0,
             activeFile:-1,
             strDate: moment(new Date().setDate(new Date().getDate() - 7)).format("MM-DD-YYYY"), //"01/"+moment().format("DD")+"/"+moment().format("YYYY"),
             endDate:new Date(),
             grid:true,
             expiry:0,
             files:"",
    ...
    getExpDate(rowInfo){
      ...
      getRecords(){
        ...
        .then((res)=>{
            ...
            this.setState({data:res.data.ResponseCollection,backData:res.data.ResponseCollection,loading:false},()=>{
            })
          }else{
      ...
      setTable(){
        ...
        let obj = {
          Header: "#",
          Cell: (rowInfo) => <ComCheck  onChange={(e)=>this.selectFile(rowInfo.original.FileID,"Table")}  containerStyle={"ComCheckMainWrp"} tickStyle={"SchedCheckTick"}></ComCheck>, // Custom cell components!
          width:25
        }
        dColumn.push(obj)
              ...
              obj={
                ...
                <div>{ 
                   ...
                   <a href={rowInfo.original.WebVisibleURL} download>
                       <img src={require("../../../Assets/Images/Media/download.jpg")} className="MediaFlColDnldImg" onClick={()=>this.downloadfile(rowInfo.original.TempName)}></img></a>
                   <img src={require("../../../Assets/Images/Media/video-preview.png")} className="MediaFlColDnldImg"></img>
                 </div> 
      ...
      setTab(index,tab){
        this.setState({tabIndex:index,contentType:tab,grid:true,loading:true,data:[]},()=>{
          this.getRecords()
        })
      }
      ...
      checkAllowed(market,key){
          ...
          if(key="All Files"){
              if(market=="NY" || market=="M10" || market=="DTTU" || market=="GLACIER"){
                  return true;
              }else{
                  return false;
              }
          }else if(key="Videos"){
              if(market=="NY" || market=="M10" || market=="DTTU" || market=="GLACIER" || market=="MX925"){
                  return true;
              }else{
                  return false;
              }
        ...
        postData("FileUpload/Update",postobj)
            .then(res=>{
                ...
                if(res.data && res.data.ReturnCode===0){
                    this.setState({dlgtitle:"Information",message:"File Uploaded Successfully",dlgType:"information",dlgEnable:true,upload:false,uplddata:[],selectedFiles:[],systProgCnt:0,uplddata:[],expDate:this.comnExpDate},()=>this.getRecords())
    ...
    upLoadFiles(){
      ...
      for(var x = 0; x<this.state.selectedFiles.length; x++) {
        ...
        this.myInterval = setInterval(()=>this.setState({systProgCnt:this.state.systProgCnt+10}),1000)
    ...
    upDateFilesold(){
      ...
      .then(res=>{
          if(res.data && res.data.ReturnCode===0){
            this.setState({dlgtitle:"Information",message:"File upload successfully",dlgType:"information",dlgEnable:true,upload:false,uplddata:[],systProgCnt:100,selectedFiles:[]},()=>this.getRecords())
          }else{
            this.setState({dlgtitle:"Information",message:"Failed please try upload again",dlgType:"information",dlgEnable:true,upload:false,uplddata:[],selectedFiles:[]})
          }
      })
    ...
    selectFile(id,type){
      ...
      if(index > 0){
        ...
        if(type === "grid" && this.state.deleteFile && this.state.deleteFile.length > 0){
    ...
    deleteFile(type){
      if(this.state.deleteFile && this.state.deleteFile.length === 0){
        this.setState({dlgtitle:"Information",message:"Select Files",dlgType:"information",dlgEnable:true})
      }else if(type === "Confirm"){
        this.setState({dlgtitle:"Confirmation",message:"Are you sure?",dlgType:"confirmation",dlgEnable:true})
      }else{
        postData("FileUpload/DeleteFile",this.state.deleteFile)
        .then(res=>{
            if(res.data && res.data.ReturnCode===0){
              this.setState({dlgtitle:"Information",message:"Deleted Successfully",dlgType:"information",dlgEnable:true})
            }else{
              this.setState({dlgtitle:"Information",message:"Something went wrong please try again.",dlgType:"information",dlgEnable:true})
            }
        })
      }
    }
        ...
        if(value!=""){
            ...
            if(this.state.backData && this.state.backData.length>0){
                ...
                this.state.backData.forEach(function (item) {
                    if(searchTag){
                        if(item.Tag.toUpperCase().includes(searchTag.toUpperCase()) && item.FileName.toUpperCase().includes(value.toUpperCase())){
    ...
    filterByTag(value){
        if(value!=""){
            ...
            if(this.state.backData && this.state.backData.length>0){
                ...
                this.state.backData.forEach(function (item) {
                    if(searchFilename){
                        if(item.FileName.toUpperCase().includes(searchFilename.toUpperCase()) && item.Tag.toUpperCase().includes(value.toUpperCase())){
            ...
            if(searchFilename){
                let tempArr=[];
                this.state.backData.forEach(function (item) {
                    if(item.FileName.toUpperCase().includes(searchFilename.toUpperCase())){
    ...
    _onMouseMove(e) {
        ...
        console.log(e.screenX +"===="+highLeft+"=="+e.screenY+"=="+highTop);
                ...
                <div className="CabDtTabMainWrp">
                  {
                    this.tabs && this.tabs.map((key,index)=>(
                        ...
                        <div className={this.state.tabIndex === index ?"CabDtTabWrp1":"CabDtTabWrp"} onClick={()=>this.setTab(index,key)} key={index}>{key}</div>
                    ))
                  }
                </div>
                {
                  this.state.upload ?
                   <div className="Loader">
                      ...
                      <div id="mediaMainDiv" ref="myImgContainer" className="MediaUpldMainWrp">
                   ...
                   <Dialog title={this.state.dlgtitle} message={this.state.message} 
                   onOk={()=>this.setState({dlgEnable:false},()=>{if(this.state.dlgType === "confirmation"){this.deleteFile()}})} 
                   onCancel={()=>this.setState({dlgEnable:false})} onHide={()=>this.setState({dlgEnable:false})} type={this.state.dlgType}></Dialog>
                }
               ...
               <div className="ScheduleTabToolWrp">
                     <div className="MediaFltWrp">
                      <div className="MediaFltWrp1">
                          ...
                          <select className="DropDwn1" onChange={(e)=>this.setState({expiry:e.target.value})} value={this.state.expiry}>
                              <option value="-1">All</option>
                              <option value="0" >Non-expired</option>
                              <option value="1">Expired</option>
                            </select>
                          <img src={require('../../../Assets/Images/Filter/apply-filter.png')} className="ScheduleupldImg" onClick={()=>this.setState({loading:true},()=> this.getRecords())}></img>
                       </div>
                       {
                         this.state.grid &&
                            <div className="MediaFltWrp1">
                              <div className='SchedHeadTxt'>Uploaded By</div>
                              <select className="DropDwn1" onChange={(e)=>this.setFields(e.target.value,"status")} value={this.state.status}>
                                  <option value="-1" >All</option>
                              </select>
                              <div className='SchedHeadTxt'>File Name</div>
                              <input style={{marginLeft:"2px"}}  onKeyUp={(e)=>this.filterByFileName(e.target.value)}></input>
                              <div className='SchedHeadTxt' style={{marginLeft:"1%"}}>Tags</div>
                              <input style={{marginLeft:"2px"}} onKeyUp={(e)=>this.filterByTag(e.target.value)}></input>
                          </div>
                       }
                    </div>
                   <div className="MadiaToolBtnContent">
                      {
                        this.state.grid ?
                         <img src={require('../../../Assets/Images/Media/tableView.png')} className="MeaiaUpldImg1" onClick={()=>this.setState({grid:false,deleteFile:[]})}></img>
                        :<img src={require('../../../Assets/Images/Media/grid.png')} className="MeaiaUpldImg1" onClick={()=>this.setState({grid:true,deleteFile:[]})}></img>
                      }
                      <img src={require('../../../Assets/Images/Upload/upload-btn.png')} className="MeaiaUpldImg" onClick={()=>this.setState({upload:true})}></img>
                      <img src={require('../../../Assets/Images/Media/delete.png')} className="MeaiaUpldImg" onClick={()=>this.deleteFile("Confirm")}></img>
                   </div>
               </div>
               {
                 this.state.grid ?

                     <div className="MediFlMainWrp" onMouseMove={this._onMouseMove.bind(this)}>

                         {
                             this.state.data && this.state.data.map((key, index) => {
                                 return (
                                     <div className={this.state.activeFile === index ? "MediaFlWrp1" : "MediaFlWrp"}
                                          onClick={() => this.setState({activeFile: index}, () => this.selectFile(key.FileID, "grid"))}
                                          key={index}>
                                         ...
                                         <div class="tooltip1">
                                             ...
                                             <div className="media_thumb_caontainer">
                                                 <img className={this.getthumbClass(key.ContentTypeID)}
                                                      src={this.getThumnail(key.WebVisibleURL)}></img>
                                             </div>
                                             <div className={"tooltiptext1  " + this.getSectionClass()}>
                                                 ...
                                                 <div>
                                                     <div className="detailsMian">
                                                         <div className="detailRow">
                                                             ...
                                                             <div className="detailcol_2">{key.FileName}</div>
                                                         </div>
                                                         <div className="detailRow">
                                                             <div className="detailcol_1">File name:</div>
                                                             <div className="detailcol_2">{key.TempName}</div>
                                                         </div>
                                                         <div className="detailRow">
                                                             <div className="detailcol_1">Content File Type:</div>
                                                             <div className="detailcol_2">{key.ContentFileType}</div>
                                                         </div>
                                                         <div className="detailRow">
                                                             <div className="detailcol_1">Path:</div>
                                                             <div className="detailcol_2">{key.PathLocation}</div>
                                                         </div>
                                                         <div className="detailRow">
                                                             <div className="detailcol_1">File Size:</div>
                                                             <div className="detailcol_2">{key.FileSize}</div>
                                                         </div>
                                                         <div className="detailRow">
                                                         ...
                                                         <div className="detailRow">
                                                             ...
                                                             <div
                                                                 className="detailcol_2">{moment(key.Dated).format("DD-MM-YYYY HH:mm:ss")}</div>
                                                         </div>
                                                         <div className="detailRow">
                                                             <div className="detailcol_1">Date Created:</div>
                                                             <div
                                                                 className="detailcol_2">{moment(key.FileDate).format("DD-MM-YYYY HH:mm:ss")}</div>
                                                         </div>
                                                         <div className="detailRow">
                                                             <div className="detailcol_1">Expiry Date:</div>
                                                             <div
                                                                 className="detailcol_2">{moment(key.ExpDate).format("DD-MM-YYYY HH:mm")}</div>
                                                         </div>
                                                         <div className="detailRow">
                                                         ...
                                                         <div className="detailRow">
                                                             ...
                                                             <div className="detailcol_2">{key.ContractID}</div>
                                                         </div>
                                                         <div className="detailRow">
                                                             <div className="detailcol_1">Impressions:</div>
                                                             <div className="detailcol_2">{key.Impressions}</div>
                                                         </div>
                                                     </div>
                                                 </div>
                                             </div>
